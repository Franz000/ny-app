...
const char* serverName = "http://192.168.43.56:3500/api/sensor/esp32server/data";
...
void setup()
{
  ...
  if ((WiFi.status() == WL_CONNECTED)) { 
    ...
    String pacienteJSON =  "{ \"token\": \""+ token + "\"}";
    int httpCode = http.POST(pacienteJSON);   
...
void loop()
{
  ...
  if (millis() - tsLastReport > REPORTING_PERIOD_MS) {
    heart = random(300);//pox.getHeartRate();
    spo2 = random(300);//pox.getSpO2();
    enviar = 1;
  ...
  for (;;) {
      if (WiFi.status() == WL_CONNECTED) {
        if (enviar==1) {
          ...
          /*String httpRequestData = "api=" + token + "&hr=" + heart
                                   + "&spo2=" + spo2 + "&pa="+id_paciente;*/

          //Serial.print("httpRequestData: ");
          //Serial.println(httpRequestData);
          String datoJSON = "{\"token\" : \"" + token + "\", \"ecg\":"+ heart +", \"spo2\":"+spo2+",\"temp\":"+ 23 +", \"idPa\":"+id_paciente+"}";

          int httpResponseCode = http.POST(datoJSON);
